...
def generate_summary(self, summary_id: int) -> None:
    ...
    try:
        summary_object = db_session.execute(select(db_models.Summary).where(db_models.Summary.id == summary_id).limit(1)).first()
    ...
    if summary_object.processed:
        logger.info(f'Summary already processed for id {summary_id}, skipping generation')
        return
