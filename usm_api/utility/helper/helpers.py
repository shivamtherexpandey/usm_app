...
class Helper:
    ...
    @staticmethod
    def is_webpage_with_content(url: str, timeout: int = 10, min_content_length: int = 100) -> Dict[str, any]:
        """
        ...
        try:
            ...
            try:
                ...
                if head_response.status_code != 200:
                    result['details'] = f'HTTP status {head_response.status_code} - not accessible'
                    return result
                ...
                if not is_webpage_content_type(content_type):
                    result['details'] = f'Content type "{content_type}" is not a webpage'
                    return result
            ...
            try:
                ...
                if get_response.status_code != 200:
                    result['details'] = f'HTTP status {get_response.status_code} - not accessible'
                    return result
                ...
                if is_webpage and has_content:
                    result['details'] = f'Webpage with {result["content_length"]} bytes of content'
