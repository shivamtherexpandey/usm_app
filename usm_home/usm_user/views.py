...
class SignupView(APIView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(
            email=validated_data["username"], is_active=True
        ).exists():
            return Response(
        ...
        complementary_free_subscription = SubscriptionPlan.objects.get(
            id=1, is_active=True
        )
        subscription = Subscription.objects.create(
            plan=complementary_free_subscription, is_active=True
        )
